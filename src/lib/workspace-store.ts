...
import type { Category, CategoryIcon, Goal } from "./types";
...
export async function loadWorkspace(ownerId: string) {
  const api = client();
  const [{ data: categoryRows, error: categoryError }, { data: goalRows, error: goalError }] = await Promise.all([
    api.from("categories").select("id,name,short_label,color,icon_key,is_default").eq("owner_id", ownerId).order("sort_order"),
    api.from("goals").select("id,title,category_id,current_value,target_value,unit,due_day,status,week_start").eq("owner_id", ownerId).order("sort_order"),
  ]);
